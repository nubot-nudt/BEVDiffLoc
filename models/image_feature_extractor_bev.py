...
@file: image_feature_extractor_bev.py
...
from models.decoders import DecoderLinear_bev
# DINOv2
...
def create_decoder(decoder_cfg):
    ...
    decoder = DecoderLinear_bev(**decoder_cfg)
...
class RangeViT(nn.Module):
    def __init__(
        ...
        resnet_encoder = resnet_models.resnet34(pretrained=True) #resnet34
        layers = list(resnet_encoder.children())[:-4]
    ...
    def forward(self, im):
        ...
        for i in range(len(self.angles)):
            ...
            grid = F.affine_grid(aff, torch.Size(im.size()),align_corners=True).type(im.type())
            
            # input warp
            warped_im = F.grid_sample(im, grid,align_corners=True,mode='bicubic')
                                    
            # cnn backbone feature
            out = self.resnet_encoder(warped_im) 
            ...
            aff[:,1,0]=-torch.sin(self.angles[i])
            aff[:,1,1]=torch.cos(self.angles[i])
            grid = F.affine_grid(aff, torch.Size(im1_init_size),align_corners=True).type(im.type())
        ...
        B,C,H,W = im.size()
        grid = F.affine_grid(aff, torch.Size((B, C, H//8+1, W//8+1)),align_corners=True).type(im.type())#,align_corners=True)
        ...
        x, skip = self.encoder(im, return_features=True) #[257, 384]

        # remove CLS tokens for decoding
        num_extra_tokens = 1
        x = x[:, num_extra_tokens:]

        pred_mask = self.decoder(x, (H, W), skip)
        # sigmoid
...
class ImageFeatureExtractor_bev(nn.Module):
    def __init__(
            self,
        ...
        if pretrained_path is not None:
            ...
            elif pretrained_path == 'resnet34_pertrain.pth':
    ...
    def forward(self, *args):
        # print("return",self.rangevit(*args)[0].shape, self.rangevit(*args)[1].shape) [6, 384] [6, 1, 32, 512]
